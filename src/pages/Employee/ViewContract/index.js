...
export default function ViewContract() {
    const [employeeContractAndSalaryDetail, setEmployeeContractAndSalaryDetail] = useState(0)

    const {employeeContractAndSalaryId} = useParams()

    const getEmployeeContractAndSalaryDetail = async() => {
        if(1){
            const employeeContractAndSalaryRes = await employeeContractAndSalaryAPI.getContractAndSalary(employeeContractAndSalaryId)
            if ( employeeContractAndSalaryRes?.status === 200){
                setEmployeeContractAndSalaryDetail(employeeContractAndSalaryRes?.data)
            }
        }      
    }

    useEffect(() => {
        getEmployeeContractAndSalaryDetail()
    }, [])
                ...
                <div id="contract1">
                    <div data-toggle="collapse" href="#collapse1" aria-expanded="true">
                        <div aria-expanded="true" class="title-list">
                            <h5>Hợp đồng mã</h5>
                            <p>{employeeContractAndSalaryDetail?.maHopDong?.id || ''}</p>
                        </div>
                    </div>
                        ...
                        <div class="row fied-data-row">
                            <div>
                                ...
                                <table>
                                    <tr>
                                        <th>Mã nhân viên:</th>
                                        <td>{employeeContractAndSalaryDetail?.maHopDong?.maNV || ''}</td>
                                    </tr>
                                    <tr>
                                        <th>Mã hợp đồng:</th>
                                        <td>{employeeContractAndSalaryDetail?.maHopDong?.loaiHopDong?.maLoaiHopDong || ''}</td>
                                    </tr>
                                    <tr>
                                        <th>Tên hợp đồng:</th>
                                        <td>{employeeContractAndSalaryDetail?.maHopDong?.loaiHopDong?.tenLoaiHopDong || ''}</td>
                                    </tr>
                                    <tr>
                                        <th>Ngày hiệu lực:</th>
                                        <td>{employeeContractAndSalaryDetail?.maHopDong?.ngayHieuLuc || ''}</td>
                                    </tr>
                                    <tr>
                                        <th>Ngày hết hạn:</th>
                                        <td>{employeeContractAndSalaryDetail?.maHopDong?.ngayHetHan || ''}</td>
                                    </tr>
                                    <tr>
                                        <th>Trạng thái:</th>
                                        <td>{employeeContractAndSalaryDetail?.maHopDong?.trangThai ? 'Đang Hoạt Động' : 'Đã Nghỉ Việc'}</td>
                                    </tr>
                                </table>
                            </div>
                            <div>
